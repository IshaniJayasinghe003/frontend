...
export default function ForgetPasswordPage() {
    ...
    const [otp, setOtp] = useState(""); // You forgot to declare otp
    ...
    async function resetPassword() {
        ...
        try {
            await axios.post(import.meta.env.VITE_BACKEND_URL + "/api/users/reset-password", {
                email: email,
                otp: otp,
                newPassword: newPassword // Fixed typo: newPaaword → newPassword
            });
            ...
            {!emailSent && (
                <div className="bg-primary w-[500px] h-[500px] shadow-2xl flex flex-col items-center justify-center gap-[20px] rounded-[30px]">
                    ...
                    <input
                        ...
                        className="w-[350px] h-[40px] border border-white rounded-xl text-center" // fixed "borrder"
                        onChange={(e) => setEmail(e.target.value)}
                    />
                    <button
                        onClick={sendOTP} // Added onClick
                        className="w-[350px] h-[40px] bg-blue-500 rounded-xl text-white text-lg mt-5 hover:bg-blue-500"
                    >
                        Send OTP
                    </button>
                </div>
